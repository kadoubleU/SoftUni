function changeBureau(input) {
    ...
    let chineseYuansNumber = Number(input[1]);
    let commision = Number(input[2]);

    let bitcoinToBgn = bitcoinsNumber * 1168;
    let chineseYuansToDollar = chineseYuansNumber * 0.15;
    let dollarToBgn = chineseYuansToDollar * 1.76;

    let sum = bitcoinToBgn + dollarToBgn;
    
    let bgnToEur = sum / 1.95;

    let commisionInEur = bgnToEur * (commision / 100);

    let endSum = bgnToEur - commisionInEur;
